...
const Int64 = require('node-int64-gas');
...
class ParquetCursor {
   ...
   */
  async next() {
    if (this.rowGroupInnerIndex == this.rowGroup.length) {
    ...
    this.rowGroupInnerIndex++;
    return this.rowGroup[this.rowGroupInnerIndex-1];
  }
...
class ParquetReader {
  ...
  /**
   * Open the parquet file from a url using the supplied fetch module
   * This function returns a new parquet reader
   */
  static async openUrl(fetch, url, params, options) {
    let envelopeReader = await ParquetEnvelopeReader.openUrl(fetch, url, params, options);
...
class ParquetEnvelopeReader {
  ...
  // This is designed for GAS's UrlFetchApp, which is synchronous when run in the GAS environment.
  static async openUrl(fetch, url, params, options) {
    params = params || {}
    let defaultHeaders = params.headers || {};

    let filesize = async () => new Promise((resolve, reject) => {
      let resp = fetch(url, params);
      if (resp.getResponseCode() == 200 || resp.getResponseCode() == 206) {
        let headers = Object.fromEntries(
          Object.entries(resp.getHeaders()).map(([k, v]) => [k.toLowerCase(), v]));
        resolve(headers['content-length']);
      } else {
        reject(`Response Code: ${resp.getResponseCode()}. Error Message: ${resp.getContentText()}`);
      }
    });

    let readFn = (offset, length, file) => {
      if (file)
        throw new Error('File URL append not supported')

      let range = `bytes=${offset}-${offset + length - 1}`;
      let headers = Object.assign({}, defaultHeaders, { range });
      let innerParams = Object.assign({}, params, { headers });
      return new Promise((resolve, reject) => {
        let resp = fetch(url, innerParams);
        if (resp.getResponseCode() == 200 || resp.getResponseCode() == 206) {
          // The buffer does not help, but we can reuse what has already been built.
          resolve(Buffer.from(resp.getContent()));
        } else {
          reject(`Response Code: ${resp.getResponseCode()}. Error Message: ${resp.getContentText()}`);
        }
      });
